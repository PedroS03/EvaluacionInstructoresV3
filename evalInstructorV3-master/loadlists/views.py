...
def createFinalPorCoordinacion(request):
    inform2 = []
    coordinacion = request.GET.get('coordinacion')
    sqlquery = "SELECT * FROM VRESULTADOSXCOORDINACION WHERE COORDINACION = ?"
    informe2 = call_db_all(sqlquery, coordinacion)

    for info2 in informe2:
        info2 = list(info2)
        diction2 = {"DOCAPRENDIZ":info2[0], "DOCINSTRUCTOR":info2[1],"COORDINACION":info[2],
    ...
    print("Datos obtenidos de la consulta:", informe2)
    print("DataFrame creado:", dfdocumento)
